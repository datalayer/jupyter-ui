...
console.log('🔍 Checking OSS Datalayer server status...');
...
(async () => {
  try {
    ...
    if (res.ok) {
      console.log('✅ OSS server is running at', OSS_SERVER_URL);
    ...
    console.warn('⚠️  OSS server responded with status:', res.status);
    console.warn(
    ...
    if (err.name === 'AbortError') {
      console.error('⏱️  OSS server check timed out after', TIMEOUT_MS, 'ms');
    } else {
      console.error('❌ OSS server is not accessible:', err.message);
    }
