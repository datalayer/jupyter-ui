...
```tsx
import { JupyterReactTheme, Jupyter, Cell } from '@datalayer/jupyter-react';

function App() {
  return (
    <JupyterReactTheme>
      <Jupyter startDefaultKernel={true}>
        <Cell source="print('Hello from Jupyter!')" />
      </Jupyter>
    </JupyterReactTheme>
  );
}
...
- [Next.js Integration](../nextjs) - For server-side rendering
- [Docusaurus Integration](../docusaurus) - For documentation sites
- [Embed Package](../../components/embed) - For simple HTML embedding
