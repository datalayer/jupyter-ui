...
| `variant` | `'jupyter' \| 'marimo'`     | Shown as a badge; `marimo` marks a reactive kernel |

### A Marimo kernel

A `Kernel` made with `variant: 'marimo'` holds Marimo's dataflow graph: every cell, output and notebook on it is reactive — running one runs the cells that read what it defined — and `kernel.marimo` answers for the graph (`register`, `remove`, `plan`, `graph`, `react`). The kernel is an ordinary one with `marimo` installed; the indicator shows the variant.
...
await kernel.ready;
const graph = await kernel.marimo?.graph();
```
