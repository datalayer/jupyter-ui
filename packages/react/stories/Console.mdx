...
you can simply set `browserKernelModule` to `true`:

<Canvas of={ConsoleStories.InBrowser} source={{code: `<Jupyter
...
<Canvas of={ConsoleStories.InBrowserJS} source={{code: `<Jupyter
    browserKernelModule={import('@jupyterlite/javascript-kernel-extension')}
...
- `browserKernelModule`: Should be set with the dynamic import of the kernel NPM package
