...
const ViewerUrl = () => {
  ...
  const [nbformatUrl, setNbformatUrl] = useState<string>();
  useEffect(() => {
    setNbformatUrl(notebookExample.url);
  }, [notebookExample]);
  return (
    <>
      <Box m={3}>
        <Jupyter startDefaultKernel={false} disableCssLoading={true}>
              ...
              <ActionList showDividers>
                <ActionList.Group title="Visualisation">
                  {visualisationExamples.map(example => (
                    <MenuLine
                      notebookExample={example}
                      icon={<NetworkIcon colored />}
                      setNotebookExample={setNotebookExample}
                      key={example.url}
                    />
                  ))}
                </ActionList.Group>
          ...
          {nbformatUrl && <Viewer nbformatUrl={nbformatUrl} outputs={true} />}
        </Jupyter>
      </Box>
    </>
  );
