...
/**
 * A row of columns: a CSS grid whose children are `LayoutItemNode`s.
 *
 * `templateColumns` is the grid template, e.g. `1fr 1fr` for two equal
 * columns or `1fr 2fr` for a narrow one beside a wide one. After the
 * Lexical playground's layout node.
 *
 * @module nodes/LayoutContainerNode
 */
