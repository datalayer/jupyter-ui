...
export const readBlockTool: ToolDefinition = {
  ...
  description:
    "Read a single block from the currently open Lexical document by its block_id. Returns the block with: block_id, block_type (e.g. 'heading', 'paragraph', 'jupyter-cell'), source (content as string), metadata (properties like level, language). Use block_id values from readAllBlocks. Works on active .lexical file.",
  ...
  parameters: {
    ...
    properties: {
      id: {
        type: 'string',
        description:
          'The block_id of the block to read (from readAllBlocks result)',
      },
    },
    required: ['id'],
  },
